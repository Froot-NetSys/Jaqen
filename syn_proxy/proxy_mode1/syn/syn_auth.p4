...
action compute_syn_cookie2() {
    modify_field_with_hash_based_offset( sa_metadata.cookie2, 0, syn_cookie_hash_flc2, 2147483648 );
}

action compute_syn_cookie3() {
    modify_field_with_hash_based_offset( sa_metadata.cookie3, 0, syn_cookie_hash_flc3, 2147483648 );
}

action compute_syn_cookie4() {
    modify_field_with_hash_based_offset( sa_metadata.cookie4, 0, syn_cookie_hash_flc4, 2147483648 );
}
...
action verify_syn_cookie() {
    subtract( sa_metadata.cookie1, tcp.seqNo, sa_metadata.cookie1 );
    subtract( sa_metadata.cookie2, tcp.seqNo, sa_metadata.cookie2 );
    //subtract( sa_metadata.cookie3, tcp.seqNo, sa_metadata.cookie3 );
    //subtract( sa_metadata.cookie4, tcp.seqNo, sa_metadata.cookie4 );
}
...
action generate_syn_cookie() {
    modify_field_with_hash_based_offset( sa_metadata.temp32b_3, 0x0, syn_cookie_hash_flc1, 2147483648 );
}
...
blackbox stateful_alu sa_bloom_filter_alu_1{
    ...
    condition_hi : sa_metadata.syn_or_rst == 2;
...
table receive_other_table {
    actions { receive_other; }
    default_action: receive_other; 
    size: 1;
}
...
// table generate_ack_packet_table {
//     actions { generate_ack_packet; }
//     size: 1;
// }

// action generate_rst_packet() {
//     add( sa_metadata.temp32b_3, tcp.ackNo, 1); 
...
action generate_temp_meta() {
    add(tcp.ackNo, tcp.seqNo, 65536); // a number out of the TCP window 
...
control process_syn_authentication {
    //apply( syn_auth_table ); // check TCP packet type 

    //apply( calc_tcp_len_table_1 ); 
    //apply( calc_tcp_len_table_2 ); 
    //apply( calc_tcp_len_table_3 ); 
    
    if ( ipv4.protocol == 0x06 ) { // TCP = 0x06 
         if ( tcp.ctrl == 0b000100) { 
            apply ( receive_rst_table );
        }   
        else if (tcp.ctrl == 0b000010 ) {
            apply ( receive_syn_table );
        } 
	else{
		apply ( receive_other_table ); 
	}    
    } else{
        apply( receive_other_table );
    }
...
/* For RST packets */
    if ( sa_metadata.syn_or_rst == 2 ) {
        // apply( sa_nonce_table1 );
        // apply( sa_nonce_table2 );
        // apply( sa_nonce_table3 );
        // apply( sa_nonce_table4 );
...
/* For SYN packets, check against the whitelist
or for matched RST packets, update the whitelist */
    if( sa_metadata.syn_or_rst == 1 or 
        ( sa_metadata.syn_or_rst == 2 and sa_metadata.cookie_match == 1 ) ) {
    ...
    if( sa_metadata.syn_or_rst == 1 ) {
	    ...
	    apply( set_in_bf_table ); 
	}
    }

    // if ( sa_metadata.syn_or_rst == 2 and sa_metadata.cookie_match == 1 ) {
