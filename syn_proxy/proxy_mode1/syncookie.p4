...
//#define CLIENT_IP 0x04000002
//#define ATTACKER_PORT_A 28
//#define ATTACKER_A_IP 0x1C000004
//#define ATTACKER_PORT_B 36
//#define ATTACKER_B_IP 0x24000001
...
/*
// For now, hardcode the client and server ports 
const bit<9> CLIENT_PORT=130; 
const bit<32> CLIENT_IP=0x82000005; //130.0.0.5
...
control ingress {
	...
	else if ( sa_metadata.syn_or_rst == 2 ) { //RST packet, update BF and then drop  
		apply(drop_table); 
	} 
	else if (sa_metadata.syn_or_rst == 0) { // forward all other packets to server  
		apply(route_to_server_table); 
	}
    }
